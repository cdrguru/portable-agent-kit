...
4. Unified Setup: Run `./.agent/tools/bin/pack-init.sh` to bootstrap a repository
...
### Agent: gemini_auditor (Lead Solutions Architect)

- **Purpose**: Ensure architectural integrity, security, and alignment with requirements
- **Note**: Powered by Gemini CLI; requires `gemini-cli-init` skill to set up
- **Rules**:
  - Focus on CRITIQUE, not code generation
  - Verify plan aligns with task objectives
  - Flag security concerns and architectural anti-patterns
  - Output: APPROVE / REJECT / NEEDS_INFO decision
- **Triggers**: Before implementation begins, after plan is drafted
- **Invoke**: `python3 tools/utilities/gemini_audit.py --auto`

### Agent: claude_code (Builder / Executor)

- **Purpose**: Implement code changes with full workspace access via Claude Code CLI
- **Note**: Anthropic's Claude Code; uses `claude-code-init` skill to bootstrap sessions
- **Rules**:
  - Default to safe execution without waiting
  - Use TodoWrite for multi-step task tracking
  - Log handoffs via `update_agent_conversation_log.py`
  - Hand off to `gemini_auditor` for architectural review when needed
- **Triggers**: `[ ]` items assigned to "claude_code" or "builder" in task list
- **Skills**: `claude-code-init`, `claude-code-handoff`
